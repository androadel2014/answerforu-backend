...
module.exports = function registerMarketplace(opts) {
  ...
  safeAlterTable(
    `ALTER TABLE marketplace_listings ADD COLUMN created_by INTEGER`
  );
  ...
  function ensureUnifiedListingsTable() {
    ...
    addCol("public_id", "INTEGER");
    addCol("created_by", "INTEGER");
  }
  ...
  /* =====================
     ...
     - stats for ALL listings created by user
     - returns avg + count + breakdown
     Endpoint:
  ...
  app.get(
    "/api/users/:userId/ratings-stats",
    authOptional,
    async (req, res) => {
      try {
        ...
        // ✅ Make sure we use BOTH user_id and created_by for safety (old dbs)
        // ✅ Reviews table uses listing_id = marketplace_listings.id (internal)
        const baseWhere = `
        (COALESCE(m.created_by, m.user_id) = ? OR COALESCE(m.user_id, m.created_by) = ?)
      `;
        ...
        // ✅ breakdown
        const bdRows = await all(
          db,
          `
        SELECT r.stars AS stars, COUNT(*) AS c
        FROM marketplace_listings m
        JOIN marketplace_reviews r
          ON r.listing_type = m.type AND r.listing_id = m.id
        WHERE ${baseWhere}
        GROUP BY r.stars
        `,
        ...
        for (const x of bdRows || []) {
          const s = String(x.stars);
          if (breakdown[s] != null) breakdown[s] = Number(x.c || 0);
        }

        // ✅ (optional) latest reviews (for showing list under stats)
        const latest = await all(
          ...
          SELECT
            ...
            u.id AS reviewer_id,
            COALESCE(up.display_name, up.username, u.username, u.email, 'User') AS reviewer_name,
            COALESCE(up.username, u.username, '') AS reviewer_username,

            m.title AS item_title,
            m.type  AS item_type,
            COALESCE(m.public_id, m.id) AS item_id
        ...
        return res.json({
          ...
          breakdown,
          latest: (latest || []).map(mapReviewRow),
        });
      } catch (e) {
        console.error("[ratings-stats]", e);
        return res.status(500).json({ error: "Failed to load ratings stats" });
      }
    }
  );

  app.get("/api/user/:userId/ratings-stats", authOptional, (req, res) => {
    req.url = `/api/users/${req.params.userId}/ratings-stats`;
    return app._router.handle(req, res, () => {});
  });

  /* =====================
     PROFILE ALL REVIEWS (FULL LIST)
     - returns ALL reviews for ALL listings created by user
     Endpoint:
       GET /api/users/:userId/reviews
       GET /api/user/:userId/reviews  (alias)
  ===================== */

  app.get("/api/users/:userId/reviews", authOptional, async (req, res) => {
    try {
      const userId = toInt(req.params.userId);
      if (!userId) return res.status(400).json({ error: "Bad userId" });
      ...
      console.error("[users/:id/reviews]", e);
      return res.status(500).json({ error: "Failed to load reviews" });
    }
  });

  // alias
  app.get("/api/user/:userId/reviews", authOptional, (req, res) => {
    req.url = `/api/users/${req.params.userId}/reviews`;
    return app._router.handle(req, res, () => {});
  });
  ...
  function mapListingRow(x) {
    ...
    return {
      ...
      // ✅ owner (old + new DB safe)
      created_by: Number(x.created_by || x.user_id || 0),
      user_id: Number(x.user_id || x.created_by || 0),
    ...
    try {
      ...
      const r = await run(
        ...
        INSERT INTO marketplace_listings
          (user_id, created_by, type, title, category, state, city,
           link, contact, address, phone, website,
           platform, topic,
...
========================================================= */
  ...
  // ✅ if missing (legacy places/groups), auto-create a mirror row in marketplace_listings
  async function resolveInternalListingId(kind, shortId) {
    const k = safeKind(kind);
    const sid = Number(shortId);
    if (!sid) return null;

    // 1) try match by public_id first, then id
    try {
      const row = await get(
        db,
    ...
    // 2) legacy mirror (places/groups only)
    if (k !== "places" && k !== "groups") return null;

    try {
      const legacyTables =
        k === "places"
          ? ["community_places", "places"]
          : ["community_groups", "groups"];

      let legacy = null;
      for (const t of legacyTables) {
        try {
          const r = await get(db, `SELECT * FROM ${t} WHERE id = ? LIMIT 1`, [
            sid,
          ]);
          if (r) {
            legacy = r;
      ...
      const title =
        safeTrim(legacy.title || legacy.name || legacy.place_name) ||
        (k === "places" ? "Place" : "Group");

      const state = safeTrim(legacy.state) || null;
      const city = safeTrim(legacy.city) || null;
      const address = safeTrim(legacy.address) || null;
      const phone = safeTrim(legacy.phone) || null;
      const website =
        safeTrim(legacy.website || legacy.link || legacy.url) || null;
      const link = safeTrim(legacy.link || legacy.url) || null;
      const contact =
        safeTrim(legacy.contact || legacy.phone || legacy.email) || null;

      const images =
        safeJsonParse(legacy.images_json) || safeJsonParse(legacy.images) || [];
      ...
      const ins = await run(
        ...
        `
        INSERT INTO marketplace_listings
          (user_id, created_by, type, title, state, city,
           address, phone, website, link, contact,
           images_json, data_json, status, is_active, public_id, created_at, updated_at)
        VALUES
          (?, ?, ?, ?, ?, ?,
           ?, ?, ?, ?, ?,
           ?, ?, 'approved', 1, ?, datetime('now'), datetime('now'))
        `,
        [
          ownerId,
          ownerId,
          ...
          JSON.stringify({ ...(legacy || {}), _legacy: true, _legacy_kind: k }),
          sid, // ✅ public_id = legacy id
        ]
      );

      return ins?.lastID ? Number(ins.lastID) : null;
    } catch (e) {
      console.error("[resolveInternalListingId mirror]", e);
      return null;
    }
  }
  ...
  async function handleReviewsAlias(req, res) {
    try {
      ...
      // ✅ accept prefixed too: service_5 / place_10 ...
      if (rawId.includes("_")) {
        const pref = await parsePrefixedId(rawId);
        if (pref) {
          shortId = Number(pref.shortId || 0);
          internalId = Number(pref.internalId || 0);
        }
      } else {
        shortId = Number(rawId);
        if (shortId) internalId = await resolveInternalListingId(kind, shortId);
      }
  ...
  async function parsePrefixedId(raw) {
    ...
    const shortId = toInt(numStr);
    if (!shortId) return null;
    if (!ALLOWED_TYPES.has(kind)) return null;

    // ✅ resolve shortId(public_id OR id) -> internal listing id
    const internalId = await resolveInternalListingId(kind, shortId);
    if (!internalId) return null;

    return { kind, internalId, shortId };
  }
  ...
  app.get("/api/marketplace/item/:id", authOptional, async (req, res) => {
    try {
      ...
      // ✅ accept prefixed like place_30, service_5 ...
      if (rawId.includes("_")) {
        const [p0, numStr] = rawId.split("_");
        const p = normType(p0);
        ...
        internalId = await resolveInternalListingId(kind, shortId);
      } else {
        const shortId = toInt(rawId);
        if (!shortId) return res.status(400).json({ error: "Bad id" });

        const resolved = await resolveInternalByAnyId("", shortId);
        internalId = resolved?.internalId ? Number(resolved.internalId) : null;
      }

      if (!internalId) return res.status(404).json({ error: "Not found" });
