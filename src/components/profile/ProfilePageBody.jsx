...
export default function ProfilePageBody({
  ...
  const cvsLabel =
    lang === "ar" ? "السير الذاتية" : lang === "es" ? "CVs" : "CVs";
  ...
  const openChatDockWithUser = async (toUserId) => {
    if (!canAct) return toast.error(tt(lang, "loginFirst"));

    const uid = Number(toUserId || 0);
    if (!uid) return toast.error("Missing user id");

    try {
      // ✅ 1) open existing (old) thread first
      const r1 = await fetch(`${API_BASE}/api/chat/threads`, {
      ...
      if (r1.ok) {
        ...
        const existing =
          items.find((t) => String(t?.other?.id || "") === String(uid)) ||
          items.find(
            (t) =>
              String(t?.other?.user_id || t?.other?.userId || "") ===
              String(uid),
          );

        if (existing?.id) {
          window.dispatchEvent(
            new CustomEvent("a4u:chat-open", { detail: { thread: existing } }),
          );
          return;
        }
      }

      // ✅ 2) if no old thread, create/get then open
      const r2 = await fetch(`${API_BASE}/api/chat/threads`, {
        method: "POST",
        headers: { "Content-Type": "application/json", ...authHeaders() },
        body: JSON.stringify({ other_user_id: uid }),
      });
      if (!r2.ok) throw new Error("thread create failed");

      const d2 = await r2.json();
      const thread = d2?.thread || d2?.item || d2;
      ...
      toast.error(lang === "ar" ? "مشكلة في فتح الشات" : "Failed to open chat");
    }
  };
            ...
            >
              ...
              {!canEdit ? (
                <>
                  ...
                  >
                    {isFollowing ? <Users size={16} /> : <UserPlus size={16} />}
                    {isFollowing ? tt(lang, "following") : tt(lang, "follow")}
                  </button>
                </>
              ) : (
