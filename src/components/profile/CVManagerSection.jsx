// src/components/profile/CVManagerSection.jsx
...
/* =========================
   Component
========================= */
export default function CVManagerSection({ lang = "en" }) {
...
${topActions.length ? topActions.join(" | ") : "✓ Looks great, nothing major"}`;
    ...
    if (finalCV.skills?.length || finalCV.languages) {
      ...
      for (let i = 0; i < skills.length; i += 2) {
        ...
        sections.push(
          new docx.Paragraph({
            children: [
              new docx.TextRun({ text: left, font, size: 24 }),
              new docx.TextRun({
  ...
  const [downloading, setDownloading] = useState({ cvId: null, format: null });
  ...
  const fetchUserCVs = async () => {
    try {
      const res = await fetch(`${API_BASE}/api/cv`, { headers: authHeaders() });
  ...
  if (loading) {
    return (
      <div className="p-10 text-center font-black text-slate-400" dir={dir}>
        {t(lang, "loading")}
      </div>
    );
  }
